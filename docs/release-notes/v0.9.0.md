...
## Chat: Turn Classification Moved Off the UI Thread

- The chat "turn state machine" — the logic that decides whether streamed output is thinking, intermediate commentary, a tool execution, or the final answer — now runs on the session's background read-loop thread instead of the UI thread. It produces already-classified operations that are merged into the message list on the UI thread; the only work left on the UI thread is the observable mutation itself
- Practically: results are classified *before* they reach the UI, rather than raw partial events being written straight to the message list and reorganised afterward
- **Fewer "Thinking" windows.** A turn that thinks, makes a brief comment, then runs a tool used to produce *two* separate Thinking bubbles. Intermediate commentary now folds into the single Thinking window for that turn, so agentic turns no longer pile up multiple thinking bubbles
